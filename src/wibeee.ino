...
void parseWibeee(String xml) {
  // Serial.println("Parsing wibeee");
  // Serial.println(xml);
  // Pinza 1 como lector de Red
  meter.activePower = inverter.wgrid = midString(xml, "<fase1_p_activa>", "</fase1_p_activa>").toFloat();
  meter.voltage = midString(xml, "<fase1_vrms>", "</fase1_vrms>").toFloat();
  meter.reactivePower = midString(xml, "<fase1_p_reactiva_ind>", "</fase1_p_reactiva_ind>").toFloat();
  meter.reactivePower += midString(xml, "<fase1_p_reactiva_cap>", "</fase1_p_reactiva_cap>").toFloat();;

  float value;
  value = midString(xml, "<fase1_factor_potencia>", "</fase1_factor_potencia>").toFloat();
  if (value > 0) { meter.powerFactor = value; meter.activePower *= -1; inverter.wgrid *= -1; meter.reactivePower *= -1; }
  meter.importActive =  midString(xml, "<fase1_energia_activa>", "</fase1_energia_activa>").toFloat() / 1000;
  
  // Pinza 2 como lector del Inversor
  inverter.wsolar = midString(xml, "<fase2_p_activa>", "</fase2_p_activa>").toFloat();
  inverter.gridv = midString(xml, "<fase2_vrms>", "</fase2_vrms>").toFloat();
