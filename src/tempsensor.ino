...
void checkTemperature(void)
{
    if (!Error.temperaturaTermo) {
        switch(config.modoTemperatura) {
            ...
            case 2: // Manual
                ...
                if ((config.flags.pwmMan || Flags.pwmManAuto) && temperaturaTermo >= config.temperaturaApagado) { if (invert_pwm > 0) { Flags.pwmIsWorking = false; shutdownPwm(false, "APAGADO TEMP MANUAL\n"); } }
                break;
    ...
    {
        if (invert_pwm > 0) { Flags.pwmIsWorking = false; shutdownPwm(false, "APAGADO TEMP ERROR LECTURA\n"); }
    }
}
