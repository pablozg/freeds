...
void AsyncMqttClient::_onMessage(char* topic, char* payload, uint8_t qos, bool dup, bool retain, size_t len, size_t index, size_t total, uint16_t packetId) {
  ...
  if (notifyPublish) {
    ...
    ///////////// Mi modificación ////////////
    char *ptempbuff { new char[len + 1]{} };
    if (ptempbuff == nullptr) {
      return;
    }
    memset(ptempbuff, 0, len + 1);
    memcpy(ptempbuff, payload, len);
    //////////////////////////////////////////

    // for (auto callback : _onMessageUserCallbacks) callback(topic, payload, properties, len, index, total);
    for (auto callback : _onMessageUserCallbacks) callback(topic, ptempbuff, properties, len, index, total);
    delete[] ptempbuff;
  }
}
